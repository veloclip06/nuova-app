...
- Landing: CTA dei tier a pagamento puntano a /registrati (per design ratificato: l'acquisto avviene da /app/piano dopo la registrazione). L'immagine OG usa font di sistema (non Archivo), accettabile per MVP
- Stripe: account/chiavi/price ID reali NON esistono ancora — il codice gira su placeholder mock in env. TUTTI gli step manuali (account, 2 price annuali, chiavi in .env.local/Vercel, endpoint webhook + whsec, config Customer Portal, Stripe Tax opzionale, dati societari [DA COMPLETARE] in privacy/termini) sono in STRIPE_SETUP.md; il loop checkout→webhook→plan flip è verificabile solo con chiavi reali (test mode + stripe CLI)
- Downgrade Completo→Essenziale via Portal con >3 paesi: grandfathering deliberato (mai delete di company_countries; il cap morde solo alla selezione) — documentato in lib/plans.ts e STRIPE_SETUP.md
- Cookie banner: PostHog parte solo dopo consenso (localStorage `cockpit-consent`); gli eventi server-side (lead, checkout_completed dal webhook) restano telemetria contrattuale dichiarata in privacy, non gated dal banner
