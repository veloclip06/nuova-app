...
Documento di handoff. Aggiornato: 10/07/2026, dopo la ratifica del nuovo pricing (2 piani annuali 149/249€).
...
- Landing: CTA dei tier a pagamento (Essenziale/Completo) puntano a /registrati (ora signup reale); manca solo il checkout Stripe (PROMPT 6). L'immagine OG usa font di sistema (non Archivo), accettabile per MVP
- PROMPT 6 (Stripe): due price ANNUALI (Essenziale 149€, Completo 249€), nessun mensile. Enum DB `companies.plan` è ancora free|starter|pro (0001_init.sql): decidere in PROMPT 6 se mappare essenziale→starter / completo→pro o rinominare l'enum — nessuna migration fatta ora. `.env.example` cita ancora NEXT_PUBLIC_STRIPE_PRICE_STARTER: da allineare in PROMPT 6
